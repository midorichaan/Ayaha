...
class mido_admins(commands.Cog):
    ...
    async def help(self, ctx, cmd=None):
        ...
        if cmd:
            ...
            if c:
                ...
                e.add_field(name=self.bot.langutil.get_lang(ctx.author.id, key='usage'), value=c.usage)
                e.add_field(name=self.bot.langutil.get_lang(ctx.author.id, key='description'), value=self.bot.langutil.get_lang(ctx.author.id, key=f'help-{c.name}'))
                e.add_field(name=self.bot.langutil.get_lang(ctx.author.id, key='aliases'), value=", ".join([f"`{row}`" for row in c.aliases]))
               
                try:
                   return await utils.reply_or_send(ctx, embed=e)
                except Exception as exc:
                    return await utils.reply_or_send(ctx, content=f"> {self.bot.langutil.get_lang(ctx.author.id, key='error')} \n```py\n{exc}\n```")
            else:
                for i in self.bot.get_command("system").commands:
                    e.add_field(name=i.usage, value=self.bot.langutil.get_lang(ctx.author.id, key=f'help-{i.name}'))
    ...
    async def reload(self, ctx, *, module=None):
        if not module:
            ...
            if excs != "":
                await utils.reply_or_send(ctx, content=f"> {self.bot.langutil.get_lang(ctx.author.id, key='failed-loads')} \n```\n{excs}\n```")
            return
        else:
            try:
    ...
    async def restart(self, ctx):
        msg = await utils.reply_or_send(ctx, content=f"> {self.bot.langutil.get_lang(ctx.author.id, key='bot-restart')}")
        ...
        try:
            ...
            await msg.edit(content=f"> {self.bot.langutil.get_lang(ctx.author.id, key='error')} \n```py\n{exc}\n```")
